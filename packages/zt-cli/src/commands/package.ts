...
async function selectPackages(options: PublishOptions): Promise<ZeroThrow.Result<Array<{name: string, path: string}>, Error>> {
  // Find repo root
...
async function checkPackageReadiness(packagePath: string, verbose: boolean = false): Promise<PackageReadiness> {
