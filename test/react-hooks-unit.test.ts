...
import { ok, err, ZeroError } from '../src/index';
...
describe('useResult hook coverage', () => {
  ...
  it('handles successful result in effect', async () => {
    ...
    // Get setters
    const [, setData] = vi.mocked((await import('react')).useState).mock.results[0].value;
    const [, setError] = vi.mocked((await import('react')).useState).mock.results[1].value;
    const [, setLoading] = vi.mocked((await import('react')).useState).mock.results[2].value;
