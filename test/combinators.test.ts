...
function toPlainResult<T, E extends Error>(result: ZT.Result<T, E>): ZT.Result<T, E> {
  return result.ok
...
describe('makeCombinable', () => {
  it('should add combinator methods to Result', () => {
    const result = ZT.makeCombinable(ZT.ok(42));
  ...
  describe('andThen', () => {
    it('chains successful operations', () => {
      const result = ZT.makeCombinable(ZT.ok(5))
        .andThen((x) => ZT.ok(x * 2))
        .andThen((x) => ZT.ok(x + 1));
    ...
    it('short-circuits on error', () => {
      const error = new ZT.Error('TEST_ERR', 'Failed');
      const result = ZT.makeCombinable(ZT.ok(5))
    ...
    it('propagates initial error', () => {
      const error = new ZT.Error('INITIAL_ERR', 'Initial failure');
      const result = ZT.makeCombinable(ZT.err<number>(error)).andThen((x) =>
        ZT.ok(x * 2)
  ...
  describe('map', () => {
    ...
    it('preserves errors', () => {
      const error = new ZT.Error('TEST_ERR', 'Failed');
      const result = ZT.makeCombinable(ZT.err<number>(error)).map((x) => x * 2);
  ...
  describe('mapErr', () => {
    it('transforms error values', () => {
      ...
      const result = ZT.makeCombinable(ZT.err(original)).mapErr(
    ...
    it('preserves success values', () => {
      const result = ZT.makeCombinable(ZT.ok(42)).mapErr(
        (_e) => new ZT.Error('NEVER', 'Should not happen')
      );
  ...
  describe('orElse', () => {
    it('returns original on success', () => {
      const result = ZT.makeCombinable(ZT.ok(42)).orElse(() => ZT.ok(99));

      expect(toPlainResult(result)).toEqual({ ok: true, value: 42 });
    });

    it('returns fallback on error', () => {
      const result = ZT.makeCombinable(ZT.err<number>(new Error('Failed'))).orElse(
        () => ZT.ok(99)
      );
    ...
    it('can chain fallbacks', () => {
      const result = ZT.makeCombinable(ZT.err<number>(new Error('Failed')))
  ...
  describe('unwrapOr', () => {
    it('returns value on success', () => {
      const value = ZT.makeCombinable(ZT.ok(42)).unwrapOr(99);
      expect(value).toBe(42);
    });

    it('returns fallback on error', () => {
      const value = ZT.makeCombinable(ZT.err<number>(new Error('Failed'))).unwrapOr(
        99
      );
  ...
  describe('unwrapOrThrow', () => {
    it('returns value on success', () => {
      const value = ZT.makeCombinable(ZT.ok(42)).unwrapOrThrow();
      expect(value).toBe(42);
    });

    it('throws error on failure', () => {
      const error = new Error('Test error');
      expect(() => ZT.makeCombinable(ZT.err(error)).unwrapOrThrow()).toThrow(error);
    });
  });
});
...
describe('pipe', () => {
  it('composes functions left to right', () => {
    ...
    const pipeline = ZT.pipe(add5, double, toString);
...
describe('collect', () => {
  it('collects all successful results', () => {
    ...
    const collected = ZT.collect(results);
    expect(collected).toEqual({ ok: true, value: [1, 2, 3] });
  });

  it('fails on first error', () => {
    const error = new ZT.Error('TEST_ERR', 'Failed at index 1');
    const results = [
      ZT.ok(1),
      ZT.err(error),
...
describe('collectAsync', () => {
  it('collects all successful async results', async () => {
    ...
    const collected = await ZT.collectAsync(promises);
    expect(collected).toEqual({ ok: true, value: [1, 2, 3] });
  });

  it('fails on first error in async results', async () => {
    const error = new ZT.Error('ASYNC_ERR', 'Async failure');
    const promises = [
      Promise.resolve(ZT.ok(1)),
  ...
  it('handles mixed timing of promises', async () => {
    const promises = [
      new Promise<ZT.Result<number, Error>>((resolve) =>
        setTimeout(() => resolve(ZT.ok(1)), 10)
      ),
      Promise.resolve(ZT.ok(2)),
      new Promise<ZT.Result<number, Error>>((resolve) => setTimeout(() => resolve(ZT.ok(3)), 5)),
    ];
...
describe('firstSuccess', () => {
  it('returns first successful result', () => {
    ...
    const first = ZT.firstSuccess(results);
